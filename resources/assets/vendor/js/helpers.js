...
const Helpers = {
    ...
    _setCollapsed(collapsed) {
        if (this.isSmallScreen()) {
            ...
            this[collapsed ? "_addClass" : "_removeClass"](
    ...
    setContentLayout(contentLayout = requiredParam("contentLayout")) {
        setTimeout(() => {
            ...
            const navbarAreaHorizontal = document.querySelector(
                ".layout-navbar > div",
            ); // For navbar area for horizontal
            ...
            if (contentLayout === "compact") {
                ...
                if (
                    containerFluid.some((el) =>
                        [contentArea, footerArea].includes(el),
                    )
                ) {
                    this._removeClass("container-fluid", [
                        contentArea,
                        footerArea,
                    ]);
                ...
                if (verticalMenu) {
                    if (
                        containerFluid.some((el) => [navbarArea].includes(el))
                    ) {
                ...
                if (
                    containerXxl.some((el) =>
                        [contentArea, footerArea].includes(el),
                    )
                ) {
                    this._removeClass("container-xxl", [
                        contentArea,
                        footerArea,
                    ]);
                    this._addClass("container-fluid", [
                        contentArea,
                        footerArea,
                    ]);
                }
    ...
    // ---
    // Init Custom Password Toggle
    initCustomPasswordToggle() {
        const toggler = document.querySelectorAll(
            ".form-custom-password-toggle i",
        );
        if (typeof toggler !== "undefined" && toggler !== null) {
            toggler.forEach((el) => {
                el.addEventListener("click", (e) => {
                    e.preventDefault();
                    const formCustomPasswordToggle = el.closest(
                        ".form-custom-password-toggle",
                    );
                    const formCustomPasswordToggleIcons =
                        formCustomPasswordToggle.querySelectorAll("i");
                    const formCustomPasswordToggleIcon =
                        formCustomPasswordToggleIcons[
                            formCustomPasswordToggleIcons.length - 1
                        ]; // Get the last <i> element
                    const formCustomPasswordToggleInput =
                        formCustomPasswordToggle.querySelector("input");

                    if (
                        formCustomPasswordToggleInput.getAttribute("type") ===
                        "text"
                    ) {
                        formCustomPasswordToggleInput.setAttribute(
                            "type",
                            "password",
                        );
                        formCustomPasswordToggleIcon.classList.replace(
                            "ri-eye-line",
                            "ri-eye-off-line",
                        );
                    } else if (
    ...
    navTabsAnimation() {
        ...
        setTimeout(() => {
            document.querySelectorAll(".nav-tabs").forEach((tab) => {
                ...
                const isVertical =
                    tab.closest(".nav-align-left") ||
                    tab.closest(".nav-align-right");
                ...
                // On Load
                setSlider(
                    tab.querySelector(".nav-link.active").closest(".nav-item"),
                );
            });
        }, 50);
    },
